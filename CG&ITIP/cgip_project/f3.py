...
def load_image():
    ...
    if image_path:
        ...
        if img is not None:
            ...
            display_image(img)
            display_original_image(img)  # Display original image
...
# Function to display the edited image in the Tkinter window
...
# Function to display the original image in the Tkinter window
def display_original_image(img):
    img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    img_pil = Image.fromarray(img_rgb)
    img_tk = ImageTk.PhotoImage(img_pil)
    img_display_original.config(image=img_tk)
    img_display_original.image = img_tk
...
def undo():
    ...
    if len(image_stack) > 1:
        ...
        display_image(img)
        adjust_window_size(img.shape[1], img.shape[0])
        display_original_image(original_img)  # Update original image display
    else:
...
img_display.pack(pady=10, side=tk.LEFT)

img_display_original = tk.Label(frame, bd=10, relief="groove")
img_display_original.pack(pady=10, side=tk.RIGHT)

# Rest of the widgets as before...
...
btn_stop_camera = tk.Button(frame, text="Stop Camera", command=stop_camera)
